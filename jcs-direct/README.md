...
- [Deployed sample application on Java Cloud Service](../jcs-deploy/README.md) //Kishore Check if this is being done as part of prev labs
...
#### Connect to Java CLoud Service's VM using ssh ####
 ...
 Use putty to connect to remote VM.
 Provide IP address of Java Cloud Service instance assigned to you.
 ![](images/02a.png)
 Browse for the private key 'privateKey' located at `/GIT_REPO_LOCAL_CLONE/cloud-utils` and open Connection  // TBD Lucas/Sachin
 ![](images/02b.png)
 Once the ssh connection established you need to login as 'opc'
...
The `StartScriptName` property shows the startup script for managed server which is `startJCSServer.sh`. Press q to quit. Open to edit this script. Use vi:
	...
	-bash-4.1$ vi /u01/data/domains/<your-weblogic-domain-name>/bin/startJCSServer.sh

Press 'i' to edit file. Insert `echo="CUSTOM PROPERTY SETTING"` before `/u01/data/domains/techco_domain/bin/startWebLogic.sh "$@"`.
 To exit from vi saving changes, press [Esc], : (colon) and type wq:

 The goal is to demonstrate how you can set e.g. custom property which is necessary for your application. The `startJCSServer.sh` should look similar:
...
Before starting the managed server check that there is no such entry in log files belong to the managed server:
	...
	-bash-4.1$ grep -C 4 'CUSTOM PROPERTY' /u01/data/domains/<your-weblogic-domain-name>/servers/<weblogic-managed-server-name>/logs/<weblogic-managed-server-name>.*
	-bash-4.1$
...
<nodemanager-host> name be found in the nodemanager.properties
example:
